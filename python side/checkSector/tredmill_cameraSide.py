...
DETECTION_CONFIDENCE = 0.8  # YOLO 탐지 신뢰도 임계값
TARGET_CLASS = 0  # Cell Phone 클래스 ID
MAX_VECTOR_LENGTH = 300  # 최대 벡터 길이 (픽셀 단위)

# 시리얼 포트 설정
ser = serial.Serial(
    port='COM8',  # windows 기준
    baudrate=115200,
...
def normalize_vector_length(length):
    ...
    """
    # 길이를 0-9 사이로 정규화
    normalized = int((length / MAX_VECTOR_LENGTH) * 5)
    # 0-9 범위로 제한
...
model = YOLO('best.pt', verbose=False)  # yolov8l.pt , yolov8m.pt, yolov8s.pt, yolov8n.pt 순서로 가벼워짐
model.model.to('cuda')
...
try:
    ...
    # MAX_FRAMES_TO_SAVE = 500
    # saved_frame_count = 0
    # SAVE_INTERVAL = 0.5  # 저장 주기 (초 단위)
    # last_saved_time = time.time() - SAVE_INTERVAL  # 바로 저장 가능하도록 초기화
    #
    # if not os.path.exists(SAVE_FOLDER):
    #     os.makedirs(SAVE_FOLDER)

    while True:
        # 디버그용 print문 수신 대기 및 출력
        ...
        if not ret:
            continue
        #
        # # 최대 저장 프레임 수보다 작으면 저장
        # current_time = time.time()
        # if saved_frame_count < MAX_FRAMES_TO_SAVE and (current_time - last_saved_time) >= SAVE_INTERVAL:
        #     filename = os.path.join(SAVE_FOLDER, f"frame_{saved_frame_count:03d}.jpg")
        ...
        # YOLO 탐지 (모든 출력 제거)
        results = model(frame, imgsz=1280, show=False, save=False, conf=DETECTION_CONFIDENCE, stream=True,
                        verbose=False)
        detections = []
        max_conf = -1
        best_detection = None
        ...
        for result in results:
            ...
            for x1, y1, x2, y2, conf, cls_id in boxes:
                if int(cls_id) == TARGET_CLASS and conf > max_conf:
        ...
        if key == 27 or key == ord('q'):
            break
        elif key in [ord('s'), ord('c'), ord('r'), ord('t')]:
            ser.write(bytes([key]))
            print(f"모드 변경 명령 전송 : {chr(key)}")
